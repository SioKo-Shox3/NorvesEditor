...
// wrong type.
std::optional<CodecError> required_string(const json& obj, const char* context, const char* key,
                                          std::string& out) {
...
std::optional<CodecError> optional_string(const json& obj, const char* context, const char* key,
...
std::optional<EngineState> engine_state_from_wire(std::string_view text) {
    if (text == "initializing") return EngineState::Initializing;
    if (text == "ready") return EngineState::Ready;
    if (text == "running") return EngineState::Running;
    if (text == "error") return EngineState::Error;
    return std::nullopt;
}

std::optional<RuntimeState> runtime_state_from_wire(std::string_view text) {
    if (text == "edit") return RuntimeState::Edit;
    if (text == "playing") return RuntimeState::Playing;
    if (text == "paused") return RuntimeState::Paused;
    if (text == "stopped") return RuntimeState::Stopped;
    if (text == "unknown") return RuntimeState::Unknown;
    return std::nullopt;
}
...
Result<HelloParams, CodecError> HelloParams::from_json(const JsonValue& value) {
    ...
    if (!obj.is_object()) {
        return fail<HelloParams>(CodecError::invalid_field("bridge.hello.params must be an object"));
    }
    if (auto e = reject_unknown_keys(obj, "bridge.hello.params",
    ...
    if (auto e = optional_string(obj, "bridge.hello.params", "clientVersion", out.clientVersion)) {
        return fail<HelloParams>(std::move(*e));
    }
    ...
    if (!pv->is_array()) {
        return fail<HelloParams>(
            CodecError::invalid_field("`bridge.hello.params.protocolVersions` must be an array"));
    }
...
Result<HelloResult, CodecError> HelloResult::from_json(const JsonValue& value) {
    const json& obj = peek(value)->json;
    if (!obj.is_object()) {
        return fail<HelloResult>(CodecError::invalid_field("bridge.hello.result must be an object"));
    }
...
Result<PlayAck, CodecError> PlayAck::from_json(const JsonValue& value) {
    const json& obj = peek(value)->json;
    if (!obj.is_object()) {
        return fail<PlayAck>(CodecError::invalid_field("runtime.play.result must be an object"));
    }
    if (auto e = reject_unknown_keys(obj, "runtime.play.result", {"accepted", "requestedState"})) {
    ...
    if (accepted_it == obj.end()) {
        return fail<PlayAck>(CodecError::invalid_field("`runtime.play.result` requires `accepted`"));
    }
    ...
    if (state_it != obj.end()) {
        if (!state_it->is_string()) {
            return fail<PlayAck>(
                CodecError::invalid_field("`runtime.play.result.requestedState` must be a string"));
        }
