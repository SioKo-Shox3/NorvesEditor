...
Result<BridgeError, CodecError> decode_error(const json& obj) {
    ...
    if (!msg_it->is_string()) {
        return err<BridgeError>(CodecError::invalid_field("`error.message` must be a string"));
    }
    ...
    if (message.empty()) {
        return err<BridgeError>(CodecError::invalid_field("`error.message` must be non-empty"));
    }
...
Result<Envelope, CodecError> decode_envelope(std::string_view wire) {
    ...
    // kind (enum).
    {
        ...
        if (kind == "request") {
            ...
            return err<Envelope>(CodecError::invalid_field(std::string("invalid kind: ") + kind));
        }
    }
