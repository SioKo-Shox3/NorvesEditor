...
> 状態: **A / A2 / B / C / U1 / U2 完了 = シーン編集全操作(create/delete/reparent/rename/
> duplicate/property-edit)+ その Undo/Redo が完成**。次は名前空間リファクタ(最後)、または
> 別スコープへ。
...
### 検証ゲート
```bash
cmake --build C:/Users/<user>/Documents/NorvesLib/build --config Debug
ctest --test-dir C:/Users/<user>/Documents/NorvesLib/build -C Debug --output-on-failure   # EntityTreeTest 含む
git -C C:/Users/<user>/Documents/NorvesLib diff --numstat   # 全行 flip でないこと
```
```powershell
git -C C:/Users/<user>/Documents/NorvesEditor status   # A2 で NorvesEditor 無変更であること
```
...
## Undo/Redo 基盤（U1 完了・U2 は今後）
エンジンが source of truth・ObjectId はエンジン採番。**エディタ側コマンドスタック**を採用
（`apps/editor/src/state/store.ts` の `undoStack`/`redoStack` + `useBridge.ts` の記録配線）。

**U1（完了、NorvesEditor `main` `4b8e766`/merge `036628e`）**: create/duplicate/reparent の
undo/redo。設計上の3つの必須是正（plan-review で純朴設計の欠陥として特定）:
- **oldParent の同期キャプチャ**: reparent 発行直前に現在の sceneTree から値としてキャプチャ
  （reducer が後から state を読む設計は live `scene.treeChanged` とレース）。
- **root 直下の正規化**: シーンルート直下の oldParentId は `undefined` に正規化し、undo は
  エンジンの nullptr=root 経路を使う（root ノード自身の id を親として渡さない）。
- **undo/redo の副作用分離**: 公開アクションラッパーを迂回し内部コマンドを直接発行（undo-of-create
  が誤って `sceneEditHistoryCleared` を発火し redo スタックを壊すのを防ぐ）。
- redo の id 不安定性は `redoCommitted` がエントリの `createdId` を新 id へ置換して解消。
- **delete undo は対象外**（`RemoveEntity` が subtree 破棄 → 再 create で ObjectId 再採番
  `AssignFreshObjectIdsRecursive` のため editor-side 逆操作が非自明。delete 実行で両スタックを
  クリア）。ツールバーボタン + Ctrl+Z/Ctrl+Y キーバインド（`BridgeRoot` メインウィンドウのみ）。
  impl-reviewer APPROVE、typecheck 3プロジェクト + vitest 435 passed。

**U2（完了、NorvesEditor `main` `7f985c0`/merge `8a72b54`）**: `object.setProperty` 経由の
編集（Property Inspector のスカラー/配列/オブジェクト編集、Phase B の Entity Name リネーム）
まで拡張。`SceneEditCommand` に `setProperty` variant を追加するのみで reducer は無変更
（全 variant 非依存のため）。`setObjectProperty` は発行前に `stateRef` から旧値を同期キャプチャ
（reparent の oldParent 捕捉と同型、`object.changed` ライブイベントとのレースに対処）。
`undo()`/`redo()` は 3-way/4-way 分岐に再構成し、末尾 `else` が setProperty 専用であることを
TypeScript の narrowing で保証（単純な分岐追加ではない）。リネームは特別扱いなし。
impl-reviewer APPROVE、typecheck + vitest 456 passed。

これで**シーン編集全操作（create/delete/reparent/rename/duplicate/property-edit）の
Undo/Redo が完成**。将来 snapshot ベースまたはエンジン側トランザクションでの delete undo
拡張は再検討の余地あり（未着手）。
