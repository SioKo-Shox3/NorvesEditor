...
> 状態: **A / A2 / B / C / U1 完了 = シーン構造編集(create/delete/reparent/rename/duplicate)
> + Undo/Redo 基盤(create/duplicate/reparent)完成**。次は U2(rename/property-edit undo)
> または名前空間リファクタ。
...
**U2（今後）**: rename/property-edit の undo（`object.setProperty` の逆操作、oldValue は現在の
snapshot から取得）。将来 snapshot ベースまたはエンジン側トランザクションでの delete undo 拡張も
再検討の余地あり。承認後に別途具体計画。
