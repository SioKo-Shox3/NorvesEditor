...
> **ステータス**: ✅ **全フェーズ完了（2026-06-30）。アセット閲覧MVP は完結。**
> **進捗**: ✅ Phase A（ワークスペース管理、34c8ec8）/ ✅ Phase B（オフライン Asset Browser/Inspector、0efb5e9）/ ✅ Phase C（Bridge プロトコル拡張 `asset.resolve` + `asset.getManifest`、cdc6750、fixtures 124→132）/ ✅ Phase D（Bridge live 健全性オーバーレイ、cf783fb）= NorvesEditor main 統合済み。✅ **Phase E（別リポ `../NorvesLib`：`AssetSystem` 列挙アクセサ + アダプタ override + `asset.read` token、NorvesLib develop に 47117de4 統合）**。各フェーズ 実装=Codex(A〜D)/Opus 実装エージェント(E) / レビュー=Claude impl-reviewer (+ A〜D は Codex review) 二重 / 全ゲート緑（NorvesEditor: verify.ps1・C++ ctest / NorvesLib: AssetSystemTest・Game ビルド・EOL flip 検査）。
> **残る手動受け入れ**: NorvesEditor を `NORVES_ENGINE_PATH=<NorvesLib>/build/Game/Debug/Game.exe` で起動し、Asset Browser に live manifest 列挙・選択時の cooked/loose 健全性が出ることを実機確認（自動 e2e は process_e2e に asset ステップ未追加のためスコープ外）。
...
> **このフェーズは別リポジトリ `C:\Users\<user>\Documents\NorvesLib` の作業。** NorvesEditor リポには一切コミットしない。NorvesLib の `AGENTS.md`/`CLAUDE.md` の規約・ポインタ整列(west 採用: ユーザー指示メモリ)・EOL flip ハザード(CRLF/LF)に従う。NorvesLib は別 PR/別ブランチで進める。**NorvesLib には作業ツリー(`.claude/worktrees/*`)が複数存在しうるため、着手時は main ツリーの最新を base にし、worktree の途中状態を誤って base にしない。**
...
## 主要参照ファイル(絶対パス)
- `C:\Users\<user>\Documents\NorvesEditor\apps\editor\src-tauri\src\bridge_state.rs`(コマンド/validate-then-forward/no-lock-across-await/REQUEST_TIMEOUT)
- `C:\Users\<user>\Documents\NorvesEditor\apps\editor\src-tauri\src\dto.rs` / `error.rs` / `lib.rs` / `protocol_names.rs`
...
- `C:\Users\<user>\Documents\NorvesEditor\bridge\cpp\engine-sdk\src\server.cpp`(dispatch ルーティング)
- `C:\Users\<user>\Documents\NorvesEditor\bridge\cpp\engine-sdk\tests\fixtures_roundtrip_test.cpp`(C++ count :192-195)
- `C:\Users\<user>\Documents\NorvesEditor\bridge\crates\norves-bridge-core\tests\fixtures_roundtrip.rs`(Rust count :124/:126/:129/:130-133/:175/:195/:221)
- `C:\Users\<user>\Documents\NorvesEditor\bridge\cpp\examples\mock-engine\mock_adapter.hpp`(8 token・触らない)
- `C:\Users\<user>\Documents\NorvesEditor\bridge\conformance\runners\alpha_method_sequence.json`(getCapabilities exact-match・不変)
- `C:\Users\<user>\Documents\NorvesEditor\bridge\spec\schema\common.schema.json`($defs 追加先)
- `C:\Users\<user>\Documents\NorvesEditor\apps\editor\src\hooks\useBridge.ts` / `state\store.ts` / `components\AppLayout.tsx`(UI 配線)
- `C:\Users\<user>\Documents\NorvesEditor\scripts\verify.ps1`(:51,:64 = ルートワークスペースのみ・src-tauri 非対象)
- `C:\Users\<user>\Documents\NorvesLib\Library\Core\Public\Asset\AssetSystem.h`(列挙アクセサ欠如・private m_Manifest)
- `C:\Users\<user>\Documents\NorvesLib\Library\Core\Public\Asset\AssetManifest.h`(GetReferenceCount/GetReference :138-139)
- `C:\Users\<user>\Documents\NorvesLib\Library\Core\Private\Asset\AssetPath.cpp`(:192-235 実挙動=絶対/`..` 受理・UNC/drive-relative のみ拒否 = 移植ではなく参照)
- `C:\Users\<user>\Documents\NorvesLib\Library\Core\Public\Asset\AssetResolveResult.h`(:14-25 status 9値 / :27-33 source 4値)
- `C:\Users\<user>\Documents\NorvesLib\Game\Bridge\NorvesLibBridgeAdapter.cpp`(capability :1120-1126・6 token)/ `.h`(override 宣言箇所)
