...
- **実装はメイン側の `implementer` サブエージェントが実行する**（Claude メイン時も Claude 内で完結。プラグイン経由の Codex 実装委譲は不安定だったため 2026-07-05 に既定から外した）。難所は implementer を上位モデルで起動して昇格する。
- **レビューは Claude＋Codex のダブルチェック（必須ゲート）**: 計画・実装とも一次＝最上位 Claude（実装者と別エージェント）、二次＝独立した Codex。二次は**直接 CLI**（`codex exec --sandbox read-only`、同期・ポーリング不要）で呼び、**プラグイン（`codex:rescue`）を経由しない**。呼び出し失敗時のみ verifier 反証で代替し、省略した旨を報告する。自己レビュー禁止（実装した本人に自分の変更をレビューさせない）。
...
3. **計画レビュー** — API 境界、依存方向、所有権、寿命、スレッド安全性、security permission、protocol compatibility、検証コマンドを確認。一次（最上位 Claude）＋独立した Codex 二次（直接 CLI）のダブルチェック。
4. **実装** — 承認された計画に沿って編集。**メイン側の `implementer` サブエージェントに委譲**（対象フェーズ・目的・許可/禁止の書き込みパス・レイヤー規約・期待する報告形式を明示して渡す）。出力は提案物として `git diff --stat` → 実 diff の順で検査してから受け入れる（スコープ逸脱に注意）。
5. **実装レビュー** — 実 diff を承認済み計画と突き合わせる。一次（最上位 Claude、実装者と別）＋クリーン文脈の Codex 二次（**直接 CLI**: `codex exec --sandbox read-only`。プラグイン経由にしない）。両者の指摘を突き合わせてから次へ。
...
- `.claude/hooks/enforce-codex-impl.mjs`（PreToolUse）が、メインスレッドによる実装ソース（`.rs/.ts/.tsx/.js/.cpp/.h` 等）の `Edit`/`Write` を物理的にブロックする。実装は `implementer` サブエージェントへ。文書・設定・protocol fixtures はブロックされない。
...
| 計画レビュー | 必ず最上位。 |
| 実装レビュー | 必ず最上位。 |
| レビュー二次（ダブルチェック） | Codex（**直接 CLI** `codex exec --sandbox read-only`・独立文脈）。 |
| 実装 | メイン側 `implementer` サブエージェント（既定は下位、難所では上位〜最上位へ昇格）。Codex への委譲は任意（直接 CLI）。 |
