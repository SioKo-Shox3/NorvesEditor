...
- C++ Bridge SDK（`bridge/cpp/`）は LF・UTF-8（BOM なし）を維持する。`bridge/cpp/CMakeLists.txt` が MSVC に `/utf-8` を付与しており、日本語 Doxygen コメントでも C4819/C2838 は発生しない。NorvesLib の BOM+CRLF 規約は本 SDK には適用しない。
